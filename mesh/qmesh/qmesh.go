...
type qmeshEntry struct {
	...
	Rotation        float32         `json:"rotation"` // degrees in JSON
}
...
func LoadFile(path string) ([]physics.MeshData, error) {
...
// In Go, we use pointer fields (*float32, *bool) to distinguish "field absent"
...
//   - enabled: true
//   - lazy: false
func LoadJSON(jsonData []byte) ([]physics.MeshData, error) {
	var file qmeshFile
	if err := json.Unmarshal(jsonData, &file); err != nil {
		return nil, err
	}

	result := make([]physics.MeshData, 0, len(file.Meshes))

	for _, mesh := range file.Meshes {
		var md physics.MeshData

		// Parse particles
		for _, p := range mesh.Particles {
			md.ParticlePositions = append(md.ParticlePositions, physics.Vec2{
				X: p.Position[0],
				Y: p.Position[1],
			})
...
func MarshalJSON(meshes []physics.MeshData) ([]byte, error) {
	...
	for i, md := range meshes {
		...
		for j := range md.ParticlePositions {
			p := qmeshParticle{
				Position: [2]float32{md.ParticlePositions[j].X, md.ParticlePositions[j].Y},
			}
