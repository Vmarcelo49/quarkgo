...
type SpatialHashing struct {
	cellSize       float32
	cellSizeFactor float32 // 1 / cellSize
	cells          map[[2]int][]*physics.Body
	bodyOldCells   map[*physics.Body]cellAABB
	pairs          []physics.BodyPair
}
...
func New(cellSize float32) *SpatialHashing {
	if cellSize <= 0 {
		cellSize = 128.0
	}
	return &SpatialHashing{
		...
		cells:          make(map[[2]int][]*physics.Body),
		bodyOldCells:   make(map[*physics.Body]cellAABB),
	}
}

// SetCellSize changes the cell size and clears all cached data.
func (s *SpatialHashing) SetCellSize(size float32) {
	s.Clear()
	s.cellSize = size
...
func (s *SpatialHashing) Clear() {
	s.cells = make(map[[2]int][]*physics.Body)
	s.bodyOldCells = make(map[*physics.Body]cellAABB)
	s.pairs = nil
}
...
func (s *SpatialHashing) Insert(b *physics.Body) {
	aabb := b.AABB()
...
func (s *SpatialHashing) Remove(b *physics.Body) {
	if old, ok := s.bodyOldCells[b]; ok {
		s.removeBodyFromCells(b, old)
...
func (s *SpatialHashing) removeBodyFromCells(b *physics.Body, cell cellAABB) {
	for cx := cell.minX; cx <= cell.maxX; cx++ {
		for cy := cell.minY; cy <= cell.maxY; cy++ {
			key := [2]int{cx, cy}
...
// bodyPairKey is a canonical key for deduplicating body pairs across cells.
// Uses reflect.ValueOf().Pointer() to get stable addresses for ordering.
type bodyPairKey struct {
	a, b uintptr
}

func newBodyPairKey(a, b *physics.Body) bodyPairKey {
	pa := reflect.ValueOf(a).Pointer()
	pb := reflect.ValueOf(b).Pointer()
	if pa <= pb {
		return bodyPairKey{a: pa, b: pb}
	}
	return bodyPairKey{a: pb, b: pa}
}
...
func (s *SpatialHashing) Pairs() []physics.BodyPair {
	s.pairs = s.pairs[:0]
		...
		for i := 0; i < n-1; i++ {
			...
			for j := i + 1; j < n; j++ {
				...
				if bodyA.AABB().Min.Y <= bodyB.AABB().Max.Y &&
					...
					if physics.CanCollide(bodyA, bodyB, true) {
						s.pairs = append(s.pairs, physics.BodyPair{A: bodyA, B: bodyB})
					}
				}
			}
		}
