...
func (sb *SoftBody) Rigidity() float32 { return sb.rigidity }
...
func (sb *SoftBody) AreaPreservingRate() float32 { return sb.areaPreservingRate }

// AreaPreservingRigidity returns the rigidity of area-preserving constraints.
func (sb *SoftBody) AreaPreservingRigidity() float32 { return sb.areaPreservingRigidity }

// TargetPreservationArea returns the target area for area preserving.
func (sb *SoftBody) TargetPreservationArea() float32 { return sb.targetPreservationArea }
...
func (sb *SoftBody) ShapeMatchingFixedRotation() float32 { return sb.shapeMatchingFixedRotation }

// ParticleSpecificMass returns the per-particle mass (if enabled).
func (sb *SoftBody) ParticleSpecificMass() float32 { return sb.particleSpecificMass }
...
func (sb *SoftBody) SetRigidity(r float32) *SoftBody { sb.rigidity = r; return sb }
...
func (sb *SoftBody) SetShapeMatchingRate(r float32) *SoftBody { sb.shapeMatchingRate = r; return sb }
...
func (sb *SoftBody) ApplyShapeMatching() {
	for _, mesh := range sb.meshes {
		...
		for n, particle := range particles {
			...
			distanceUnit := distance.Div(distanceLen)

			// Quadratic force, capped at distanceLen
			forceLinear := distanceLen * distanceLen * (0.02 * (1 + sb.rigidity)) * sb.shapeMatchingRate
			if forceLinear > distanceLen {
				forceLinear = distanceLen
			}
