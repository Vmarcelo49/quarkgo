...
// of pieces, but is fast and robust enough for real-time physics.
//
...
func crossProduct(prev, curr, next Point) float32 {
	v1x := curr.X - prev.X
	v1y := curr.Y - prev.Y
	v2x := next.X - curr.X
	v2y := next.Y - curr.Y
...
// --- Convenience wrappers for physics.Vec2 / physics.Particle ---

// ConvexPartitionFromParticles is a convenience wrapper that accepts a slice
// of *physics.Particle (using global positions) and returns convex sub-polygons
// as slices of *physics.Particle.
//
// This mirrors QMesh::DecompositePolygon in qmesh.cpp:625-665.
func ConvexPartitionFromParticles(particles []*physics.Particle) [][]*physics.Particle {
	if len(particles) < 3 {
		return nil
	}
	...
	out := make([][]*physics.Particle, len(result))
	for i, subPoly := range result {
		out[i] = make([]*physics.Particle, len(subPoly))
