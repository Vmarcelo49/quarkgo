...
func pinJointSample(scene *common.Scene, x, y float32) {
	// Ball pinned to air
	...
	// Pin ball to air
	j1 := physics.NewJoint(ball, physics.Vec2{X: x, Y: y - 24}, physics.Vec2{X: x, Y: y - 24}, nil)
	j1.SetRigidity(1.0)
	scene.World.AddJoint(j1)

	// Ball to box
	j2 := physics.NewJoint(ball, physics.Vec2{X: x, Y: y + 24}, physics.Vec2{X: x, Y: y + 48 - 16}, box)
	j2.SetRigidity(1.0)
	scene.World.AddJoint(j2)

	// Box to polygon
	j3 := physics.NewJoint(box, physics.Vec2{X: x, Y: y + 48 + 16}, physics.Vec2{X: x, Y: y + 48 + 32 - 24}, poly)
	j3.SetRigidity(1.0)
...
func springDistanceJointSample(scene *common.Scene, x, y float32) {
	...
	for i := range 6 {
		...
		if i == 0 {
			...
			j := physics.NewJoint(ball, physics.Vec2{X: x, Y: y}, physics.Vec2{X: x, Y: y}, nil)
			j.SetRigidity(1.0)
			scene.World.AddJoint(j)
		}
		if prev != nil {
			j := physics.NewJoint(prev,
				physics.Vec2{X: x, Y: y + float32(i-1)*48 + 24},
				physics.Vec2{X: x, Y: y + float32(i)*48 - 24},
				ball)
...
func grooveJointSample(scene *common.Scene, x, y float32) {
	box1 := scene.AddRectBodySized(x, y, 32, 32)
	box2 := scene.AddRectBodySized(x, y+48, 32, 32)

	// Pin top box to air
	j1 := physics.NewJoint(box1, physics.Vec2{X: x, Y: y}, physics.Vec2{X: x, Y: y}, nil)
	j1.SetRigidity(1.0)
	scene.World.AddJoint(j1)

	// Groove joint between boxes (pull-only, length 96)
	j2 := physics.NewJoint(box1,
		physics.Vec2{X: x, Y: y + 16},
		physics.Vec2{X: x, Y: y + 48 - 16},
		box2)
	j2.SetRigidity(1.0)
...
func distanceJointSample(scene *common.Scene, x, y float32) {
	...
	for i := range 6 {
		...
		if prev != nil {
			j := physics.NewJoint(prev,
				physics.Vec2{X: x, Y: y + float32(i-1)*64 + 16},
				physics.Vec2{X: x, Y: y + float32(i)*64 - 16},
				box)
			j.SetRigidity(1.0)
			scene.World.AddJoint(j)
		}
