...
func (pb *PlatformerBody) FloorMaxAngleDegree() float32    { return pb.maxFloorAngle / (physics.Pi / 180) }
...
func (pb *PlatformerBody) SetFloorMaxAngleDegree(v float32) *PlatformerBody {
        return pb.SetFloorMaxAngle(v * (physics.Pi / 180))
}
...
func (pb *PlatformerBody) GetRightWall(offset float32) CollisionTestInfo {
        ...
        if test.HasBody() {
                ...
                if physics.Abs(normalAngle) > pb.maxFloorAngle &&
                        physics.Abs(normalAngle) < (physics.Pi-pb.maxFloorAngle) {
                        return test
                }
        }
...
func (pb *PlatformerBody) GetCeiling(offset float32) CollisionTestInfo {
        ...
        if test.HasBody() {
                normalAngle := physics.AngleBetweenTwoVectors(test.Normal, pb.upDirection)
                if physics.Abs(normalAngle) > (physics.Pi - pb.maxFloorAngle) {
                        return test
                }
        }
        return CollisionTestInfo{}
...
func (pb *PlatformerBody) PostUpdate() {
                ...
                if world != nil {
                        ...
                        for _, m := range manifolds {
                                ...
                                for _, contact := range m.Contacts() {
                                        ...
                                        if physics.Abs(floorAngle) < pb.maxFloorAngle {
                                                stillFloor = true
                                                break
                                        }
                                }
        ...
        if world != nil {
                ...
                for _, m := range ceilingManifolds {
                        ...
                        for _, contact := range m.Contacts() {
                                ...
                                floorAngle := physics.AngleBetweenTwoVectors(normal, pb.upDirection)
                                if physics.Abs(floorAngle) > (physics.Pi - pb.maxFloorAngle) {
                                        pb.onCeiling = true
                                        break
                                }
                        }
        ...
        if pb.walkSide == 0 {
                // Decelerate
                if physics.Abs(pb.horizontalVelocity.X) < 0.001 && physics.Abs(pb.horizontalVelocity.Y) < 0.001 {
