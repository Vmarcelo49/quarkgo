...
func TestVec2Normalized(t *testing.T) {
	...
	v := Vec2{X: 3, Y: 4}
	n := v.Normalized()
	if got := n.Length(); math32.Abs(got-1) > 1e-6 {
	...
	if math32.IsNaN(nz.X) || math32.IsNaN(nz.Y) {
...
func TestVec2Rotated(t *testing.T) {
	...
	if math32.Abs(r.X-0) > 1e-6 || math32.Abs(r.Y-1) > 1e-6 {
...
func TestAngleToUnitVector(t *testing.T) {
	...
	if math32.Abs(v.X-1) > 1e-6 || math32.Abs(v.Y-0) > 1e-6 {
		t.Errorf("AngleToUnitVector(0) = %v, want (1,0)", v)
	}
	v = AngleToUnitVector(math32.Pi / 2)
	if math32.Abs(v.X-0) > 1e-6 || math32.Abs(v.Y-1) > 1e-6 {
...
func TestAngleBetweenTwoVectors(t *testing.T) {
	...
	if math32.Abs(got-math32.Pi/2) > 1e-6 {
...
func TestGetBisectorUnitVector(t *testing.T) {
	...
	want := Vec2{X: 1 / math32.Sqrt(2), Y: 1 / math32.Sqrt(2)}
	if math32.Abs(b.X-want.X) > 1e-6 || math32.Abs(b.Y-want.Y) > 1e-6 {
